...
|---|---|---|---|---|
| 2026-09-24 | — | the selector's menu shows `when_not_to_use` (it never did on v3); TextCounter's gains "a position in a countdown or list" | new `japanese-houses-list` × 2 runs × 3 arms; the 7 others × 1 run × 2 arms, beats reused | Target case component accuracy 17%/17% → 100%/100%. The clause alone did nothing (never shown); the `not:` line alone was 100%/17% — both are needed. Elsewhere single runs, mixed within noise: component accuracy up on 5 of 7, down on good-news (86→81) and neu (36→33); recall up on 5, down on cnbc and _synthetic. Selector input +~30% (cnbc 146k→187k). Found: SatelliteLocate on a place missing from the gazetteer kills the compile, in both arms (4 runs) |
