...
| Beat review    | `/videos/[id]/review`  | Real stages from `video_events`; progress counts beats with a resolved asset (a beat sheet has no approval field); per-beat re-roll and edit. Editing one beat: on the left a **player showing the beat as the renderer will draw it** — the same `VideoComposition`, over the real plan, with the player's whole timeline being the beat — the plan is never cut down to it (a transition is drawn by the items on both sides of a cut, a bed's ducking envelope is absolute-time), a negative-offset Sequence shifts it under the window instead (`engine/src/renderers/remotion/BeatWindow.tsx`) — so the shot, its transition, the burnt-in captions, the overlay and the audio are the render's own; `lib/beatPreview.ts` drops in the overlay the FORM holds so a component picked a second ago is visible before a compile places it. Sourcing also takes an **upload** — a still or a clip of your own for one beat, validated before it is written (extension, magic bytes, size; ffprobe adds warnings for a clip shorter than the beat or footage below frame size), stored in `uploads/` rather than `clips/` so D19 retention cannot delete it, and the item is locked so a recompile keeps it. Narration small at the top right; a **Hand-off** select (D89) offering the transitions this video's pack allows, disabled on the last beat — the player draws it the way it draws a picked overlay, and plays exactly the transition's length past the cut, because a transition lives in the handle AFTER the cut and a window that stopped at the beat's end stopped just before the thing being changed; the **overlay menu is this video's own** `cfg.style_pack_doc.overlays.allowed_components`, and picking one generates a props form from the catalog entry's prop specs, with props resolved by `lib/overlayProps.ts` the way the compiler resolves them. **Saving a beat no longer renders**: `PUT .../beats` writes the file, `POST .../render` is the manual trigger, and `render_pending` (beats.json newer than the plan) drives the banner |
